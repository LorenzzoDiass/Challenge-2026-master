...
export default function Dashboard() {
  ...
  const alto = clientes.filter((cliente) => cliente.risco === "ALTO").length;
  const medio = clientes.filter((cliente) => cliente.risco === "MÉDIO").length;
  const baixo = clientes.filter((cliente) => cliente.risco === "BAIXO").length;

  const clientesPrioritarios = clientes.filter(
    (cliente) => cliente.risco === "ALTO" || cliente.risco === "MÉDIO"
  );
      ...
      >
        ...
        <View
          style={[
            ...
            (isMobile || isTablet) && styles.cardsContainerStack,
          ]}
        >
          <View style={[styles.card, styles.redCard]}>
            <Text style={styles.cardLabel}>🔴 Alto risco</Text>

            <Text
              style={[styles.cardNumber, isMobile && styles.cardNumberMobile]}
            >
              {alto}
            </Text>
          ...
          <View style={[styles.card, styles.yellowCard]}>
            <Text style={styles.cardLabel}>🟡 Médio risco</Text>
          ...
          <View style={[styles.card, styles.greenCard]}>
            <Text style={styles.cardLabel}>🟢 Baixo risco</Text>
        ...
        <View style={[styles.section, isMobile && styles.sectionMobile]}>
          ...
          {clientesPrioritarios.map((cliente) => (
            <View
              key={cliente.id}
              style={[styles.clientCard, isMobile && styles.clientCardMobile]}
            >
              <View style={styles.clientInfoWrapper}>
                <Text style={styles.clientName}>{cliente.nome}</Text>

                <Text style={styles.clientInfo}>
                  {cliente.modelo} • {cliente.km.toLocaleString("pt-BR")}km
                </Text>
              </View>
              ...
              >
                <Text style={styles.badgeText}>{cliente.risco}</Text>
              </View>
            </View>
          ))}
        </View>

        <View style={[styles.buttonsContainer, isMobile && styles.buttonsMobile]}>
          <TouchableOpacity
            style={styles.analyticsButton}
            onPress={() => router.push("/agendamentos")}
          >
            <Text style={styles.analyticsButtonText}>Ver Agendamentos</Text>
          </TouchableOpacity>
          ...
          >
            <Text style={styles.buttonText}>Ver Todos os Clientes</Text>
          </TouchableOpacity>
          ...
          >
            <Text style={styles.analyticsButtonText}>Ver Analytics & IA</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </ImageBackground>
