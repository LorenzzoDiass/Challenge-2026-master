...
export default function Analytics() {
  ...
  const mediaKm = Math.round(
    clientes.reduce((soma, c) => soma + c.km, 0) / total
  );

  const percentualAlto = Math.round((alto / total) * 100);
  const percentualMedio = Math.round((medio / total) * 100);
  const percentualBaixo = Math.round((baixo / total) * 100);
    ...
    >
      ...
      >
        ...
        <Text style={[styles.subtitle, isMobile && styles.subtitleMobile]}>
          Inteligência aplicada à retenção de clientes no pós-venda Ford
        </Text>

        <View style={[styles.grid, isMobile && styles.gridMobile]}>
          <View style={styles.card}>
            <Text style={styles.label}>Clientes analisados</Text>
            <Text style={[styles.number, isMobile && styles.numberMobile]}>
              {total}
            </Text>
          </View>

          <View style={styles.card}>
            <Text style={styles.label}>Média de quilometragem</Text>
            <Text style={[styles.number, isMobile && styles.numberMobile]}>
          ...
          <View style={styles.card}>
            <Text style={styles.label}>Clientes em alerta</Text>
            <Text
              style={[styles.number, styles.red, isMobile && styles.numberMobile]}
            >
              {alto + medio}
            </Text>
          </View>
        ...
        <View style={[styles.twoColumns, isMobile && styles.twoColumnsMobile]}>
          ...
          <View style={styles.sectionSmall}>
            ...
            <Text style={styles.row}>
              {medio} clientes devem receber campanha preventiva.
            </Text>
        ...
        <View style={[styles.aiBox, isMobile && styles.aiBoxMobile]}>
          <Text style={[styles.aiTitle, isMobile && styles.aiTitleMobile]}>
            Sugestão da IA
          </Text>

          <Text style={styles.aiText}>{insightIA}</Text>
        </View>
        ...
        {relatorioGerado && (
          <View style={styles.reportBox}>
            <Text style={[styles.reportTitle, isMobile && styles.reportTitleMobile]}>
              Relatório gerado
            </Text>

            <Text style={styles.reportText}>
              Foram analisados {total} clientes. Atualmente, {alto} estão em
              alto risco, {medio} em médio risco e {baixo} em baixo risco.
            </Text>
            ...
            <Text style={styles.reportText}>
              Recomendação da IA: priorizar contato imediato com clientes de
              alto risco e criar campanhas preventivas para clientes em médio
              risco.
            </Text>
          </View>
        )}
